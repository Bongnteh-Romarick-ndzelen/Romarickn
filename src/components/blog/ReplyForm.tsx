...
export default function ReplyForm({ postId, parentId, onCommentSubmitted, userId }: ReplyFormProps) {
  ...
  const onSubmit = async (data: CommentFormValues) => {
    ...
    try {
      ...
      const newComment = await commentService.createReply(parentId, commentData);
  ...
  if (!userId) {
    return (
      <div className="p-6 rounded-lg bg-slate-800/30 border border-slate-700/50 text-center">
        <p className="text-slate-400 mb-4">Please log in to post a reply.</p>
      </div>
    );
  }
