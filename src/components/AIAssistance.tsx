...
export function AIAssistance() {
  ...
  useEffect(() => {
    const visited = localStorage.getItem("ai_assistant_visited");
    if (!visited) {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[440px] max-w-[440px]"
          >
            <Card className="bg-white border-2 border-slate-200/80 shadow-2xl shadow-blue-500/10 rounded-2xl overflow-hidden">
              {/* Header */}
              <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-600 px-4 py-3.5 sm:px-5 sm:py-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div className="absolute inset-0 bg-white rounded-full blur-md opacity-20" />
                      <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/10 backdrop-blur flex items-center justify-center border border-white/20">
                        <Bot className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                      </div>
                    </div>
                    <div>
                      <h3 className="text-white font-bold text-sm sm:text-base font-['Lato',sans-serif]">
                        Romarick's Assistant
                      </h3>
                      <p className="text-white/70 text-[10px] sm:text-xs flex items-center gap-1.5 font-['Lato',sans-serif]">
              ...
              {/* Quick questions */}
              <div className="px-3 py-2 sm:px-4 sm:py-2.5 bg-slate-50/50 border-b border-slate-200">
                <p className="text-[10px] sm:text-xs text-slate-500 font-bold mb-1.5 flex items-center gap-1.5 font-['Lato',sans-serif]">
                  ...
                  {quickQuestions.map((q, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-2 bg-white border-2 border-slate-200 hover:border-blue-300 hover:bg-blue-50 rounded-xl text-slate-600 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-[10px] sm:text-xs font-bold font-['Lato',sans-serif]"
                      >
              ...
              {/* Messages */}
              <div className="h-[300px] sm:h-[340px] overflow-y-auto px-3 py-3 sm:px-4 sm:py-4 space-y-3 custom-scrollbar bg-slate-50/30">
                {messages.map((message) => (
                  ...
                  >
                    <div className={`max-w-[88%] ${message.sender === "user" ? "items-end" : "items-start"} flex flex-col`}>
                      {message.sender === "ai" && (
                      ...
                      <div
                        className={`px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-2xl ${
                          message.sender === "user"
                ...
                <div className="flex items-center justify-center gap-2">
                  {socialButtons.map((social, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-2 rounded-xl bg-slate-50 border-2 border-slate-200 hover:border-blue-300 hover:bg-blue-50 transition-all duration-200"
                    >
                      <social.icon className="h-4 w-4 text-slate-600 hover:text-blue-600 transition-colors" />
                    </a>
                  ))}
                </div>
              </div>
              ...
              <div className="p-3 sm:p-4 border-t border-slate-200 bg-white">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-50 border-2 border-slate-200 text-slate-700 placeholder:text-slate-400 text-sm sm:text-base h-10 sm:h-11 px-3.5 sm:px-4 rounded-xl focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 disabled:opacity-50 transition-colors font-['Lato',sans-serif] font-semibold"
                    disabled={isProcessing}
                  />
                  <Button
                    onClick={handleSendMessage}
                    disabled={!input.trim() || isProcessing}
                    className="h-10 w-10 sm:h-11 sm:w-11 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:opacity-40 shrink-0 shadow-lg shadow-blue-600/20"
                  >
                    {isProcessing ? (
                ...
                <p className="text-[8px] sm:text-[10px] text-slate-400 text-center mt-1.5 font-['Lato',sans-serif] font-semibold">
